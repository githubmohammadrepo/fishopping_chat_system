            ...
            User.findOne({
                ...
                if (audienceRooms.length) {
                    ...
                    audienceRooms.forEach(room => {
                        socket.to('room_' + room.name).emit('addNewonlineUser', {

                            data: {
                                user: {
                                    id: socket.id,
                                    username: socket.handshake.query.user.username,
                                    groupName: socket.handshake.query.user.groupName,
                                    province_name: socket.handshake.query.user.province_name,
                                    isMe: false
                                }
                            },
                            status: {
...
io.on('connection', (socket) => {
    ...
    //send public message based on access group defined in databse
    socket.on('privateMessage', async function(message) {
        console.log(socket.handshake.query.user)
        let t = await socket.emit('privateMessage', {
            data: {
                conent: message.data.message,
                type: 'send'
            },
            status: {
                type: 'success'
        ...
        socket.to(message.to.id).emit('privateMessage', {
            data: {
                conent: message.data.message,
                type: 'recive'
            },
    ...
    socket.on("disconnecting", (reason) => {
        ...
        User.findOne({
            ...
            if (audienceRooms.length) {
                ...
                audienceRooms.forEach(room => {
                    socket.to('room_' + room.name).emit('offlineUser', {
